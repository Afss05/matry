...
//------------------------------------------------------------------------------
	...
	public function decoder($data)
	{
		...
		// Step 2: Base64 decode stage 1
		$stage1 = base64_decode($data, true);
		if ($stage1 === false) {
			return null;
		}

		// Step 3: Safely extract lengths and substrings
		...
		$stage2 = substr($stage1, 5 + $len, (int)$strlen);
		if ($stage2 === false) {
			return null;
		}

		// Step 4: Base64 decode stage 2
		$stage3 = base64_decode($stage2, true);
		if ($stage3 === false) {
			return null;
		}
...
//---------------------------------------------
function ImageName_bypref($pref){
    $random_num = $this->call_yearsec_field();
    $rand_num = $this->randomNumber(5);
    $path=$pref.$random_num.$rand_num;
    return $path;
}
...
public function getMemberCode($count){
	...
	$content=' <table style="table-layout:fixed;width:90%;width:600px;margin:0 auto;background:#fff;font-size:14px;border:2px solid #e8e8e8;text-align:left;table-layout:fixed;padding-bottom:67px;border-spacing:0">
    <tbody>
      <tr>
        <td colspan="5" style="background-color:#DD0302;padding:20px 30px 20px 30px;text-align:center;color:#fff;font-weight:bold;font-size:1.2em">
	   ...
	   <tr>
        <td colspan="5" style="padding:10px 10px 10px 10px;line-height:1.8;text-align:center;color:#000;">
...
//-------------------------------------
function Image_pref($pre){
    $random_num = $this->call_yearsec_field();
    $rand_num = $this->randomNumber(5);
    $path=$pre.$random_num.$rand_num;
    return $path;
}

//------------------------------------------------------------------------------ 
    public function register_mail($email,$name){
	$content='<table style="table-layout:fixed;width:90%;width:600px;margin:0 auto;background:#fff;font-size:14px;border:2px solid #e8e8e8;text-align:left;table-layout:fixed;padding-bottom:67px;border-spacing:0">
	<thead>
	<tr style="background-color:#DD0302; color:#fff;">
	<th colspan="5" style="padding:20px;text-align:center;font-weight:bold;font-size:1.2em" >Welcome to BharatVivaha Family.</th>
	...
	<tbody>
	<tr>
	<td colspan="5" style="padding:50px 30px 0px 30px;text-align:center;color:#000;font-weight:bold;">
	<p>Hi  '.$name.' </p>

	</tr >
	<tr >
	<td colspan="5" style="padding:20px;text-align:center;font-weight:bold;font-size:1.2em" >Kindly update your profile once you logged in and get more number of views to your profile.</td>
	</tr>
	<tr>
	<td colspan="5" style="padding:10px 30px 0px 30px;color:#000;"><p>Regards</p>
	<p>Bharatvivaha Team </p></td>
