...
<input type="hidden"  name="mother" value="<?php  if (!empty($_GET['mother'])){ if($_GET['mother']!='') { echo $_GET['mother']; } } ?>">
...
//age code is modified successfull
...
if (isset($profile_details) && ($profile_details != "")) {
    foreach ($profile_details as $item2) {
        ...
        // If DOB (Date of Birth) is stored, calculate the actual age
        if (!empty($item2->DOB) && $item2->DOB !== "0" && $item2->DOB !== "0000-00-00") {
            try {
                ...
                echo "Invalid DOB"; // Handle invalid date formats
            }
        } else {
            echo "DOB not available"; // Handle missing or invalid DOB
        }
...
if($FilePath!=""){
?>
<img src="<?php echo base_url(); ?>assets/profileimages/<?php echo $FilePath; ?>" style="max-height:80px">
<?php }else { ?>
...
<div class="col-md-4 col-sm-4  col-xs-12" style="padding-top: 21px;">
...
if($FilePath!=""){
?>
<img src="<?php echo base_url(); ?>assets/profileimages/<?php echo $FilePath; ?>"   class="img-responsive img-whp" style="height: 200px;" >
<?php }else { ?>
...
<table>
  ...
  <tr>
    <td><strong>Gender :</strong> <?php if($Gender=="M"){ echo "Male"; }else { echo "Female"; }  ?></td>
  

  </tr>
  <tr>
    <td><strong>Height :</strong> <?php if($item2->Height!="" && $item2->Height!="0"){
...
echo $item2->Height;
  ...
  <tr>
    <td><strong>Religion :</strong> <?php if($item2->ReligionId!="" && $item2->ReligionId!="0"){

$ReligionId=$item2->ReligionId;
...
echo $CasteName=$this->chsslibrary->getFieldVal(TBL__PREFIX2."religion", "Religion", "Id=".$ReligionId);
  ...
  <tr>
    <td><strong>Caste :</strong> <?php 
if($item2->CastName!="" && $item2->CastName!="0"){
$Casteid=$item2->CastName;
echo $CasteName=$this->chsslibrary->getFieldVal(TBL__PREFIX2."caste", "CasteName", "Id=".$Casteid);
  ...
  <tr>
    <td><strong>Qualification :</strong> <?php echo $Qualification; ?></td>
  </tr>
 <!-- <tr>
    <td><strong>Employed In :</strong> <?php 
$UserEmployed="";
if($item2->UserEmployed!="" && $item2->UserEmployed!="0"){
$UserEmployed=$item2->UserEmployed;
...
?> </td>
    ...
    <tr>
    <td><strong>Employee  :</strong> <?php if($item2->Occupation!="" && $item2->Occupation!="0"){

echo $item2->Occupation;
}?></td>
  </tr>
   <tr>
    <td>
...
$userid = $this->session->userdata('logged_in');
if($userid==""){ ?>
<a href="<?php echo base_url(); ?>user/login/" target="_blank"> <button type="button" class="btn btn-sm ulockd-btn-thm2 bdrs20" style="padding:10px;margin-bottom:10px;"> View Full Profile</button></a>
<?php }else{ ?>
<a href="<?php echo base_url(); ?>searchindex/search_profile/<?php echo $enid; ?>" target="_blank"> <button type="button" class="btn btn-sm ulockd-btn-thm2 bdrs20" style="padding:10px;margin-bottom:10px;"> View Full Profile</button></a>
...
<div class="row"  >
<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12" style="padding:20px;">
<?php 
...
if($link!=""){
echo '<a href="'.base_url().'"> <button type="button" class="btn btn-lg ulockd-btn-thm2 bdrs20" style="padding:10px;margin-bottom:10px;"> More</button></a>';
break;
}
}
