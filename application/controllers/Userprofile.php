...
class userprofile extends CI_Controller
{
    ...
    public function index($id=null,$memcode=null){
		...
		if($id!=""  && $memcode!="" ){
			...
			//print_r($data["profile_details"]);
			$usercaste=$data["profile_details"][0]->CastName;
			$usergender=$data["profile_details"][0]->Gender;
			$data["horoscope_details"]=$this->Admin_model->checkuserhorscop_profile($userid);
			$data["from"] = "ByUser";
			$data['related_profiles']=$this->Admin_model->related_profiles($usergender,$usercaste);
			$this->data = $data;
			$this->template['leftmenu'] = $this->load->view('left_menu', $this->data, true);$this->template['menu'] = $this->load->view('menu', $this->data, true);
    ...
    public function edit_userprofile_submit()
    {
        ...
        for ($i = 0; $i < $cpt; $i++) {
			...
			//print_r($file);die;
            $new_path                = './assets/profileimages/';
            $old_path                = $files['horos_image']['tmp_name'][$i];
            $config['upload_path']   = './assets/profileimages/';
            //$config['file_name']        = $directoy;
            //$config['allowed_types']        = 'jpg|png|pdf|doc';
            $config['allowed_types'] = '*';
    ...
    public function user_photo_delete($id=null,$memberid=null){
		...
		if($id!="" && $memberid!='' ){
			...
			redirect(base_url() . 'userprofile/user_photo_edit/'.$memberid);
    ...
    public function user_horsphoto_delete($id=null,$memcode=null ){
		...
		if($id!="" && $memcode!="" ){
			...
			$this->session->set_flashdata('message', 'Deleted successfully.');
			redirect(base_url() . "userprofile/index/".$id."/".$memcode);
