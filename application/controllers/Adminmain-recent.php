...
class adminmain extends CI_Controller
{
	...
	//verified status updation
	
	public function verifystatus(){
		$userid= $this->uri->segment(3);
		$statusid= $this->uri->segment(4);
		$res=$this->Admin_model->verifyupd($userid,$statusid);
		if($res==1){
...
$myData = $this->Admin_model->getallmemberrecords();

  // get data
        //$myData = $this->MyModel->getData();
        ...
        $header = array('Id','Name', 'ProfileFor', 'Email', 'MemberCode', 'Gender', 'DOB', 'Age', 'MotherTongue', 'CastName', 'Height', 'Qualification','Occupation', 'MonthlyIncome', 'ContactNumber', 'AlternativeNumber', 'PermenantAddress', 'State', 'City', 'PresentAddress', 'FatherName', 'MotherName', 'AddedDate', 'Rasi', 'Star','SubCaste','Totalviewedcount');
        fputcsv($file, $header);
 
        foreach ($myData as $line){
        	$stateidd=$line['StateId'];
			$cityidd=$line['CityId'];
			$raasidd=$line['Rasi'];
			$staridd=$line['Star'];
			
			if ($stateidd!="" && $stateidd!="0") {

				$line['StateId']=$this->chsslibrary->getFieldVal(TBL__PREFIX2."statemaster", "StateName", "Id=".$stateidd);
			}
			
			if ($cityidd!="" && $cityidd!="0") {



				$line['CityId']=$this->chsslibrary->getFieldVal(TBL__PREFIX2."citymaster", "CityName", "Id=".$cityidd);

			}
			if ($raasidd!="" && $raasidd!="0") {
				$line['Rasi']=$this->chsslibrary->getFieldVal(TBL__PREFIX2."rasi", "RasiName", "Id=".$raasidd);
			}
			if ($staridd!="" && $staridd!="0") {

				$line['Star']=$this->chsslibrary->getFieldVal(TBL__PREFIX2."star", "StarName", "Id=".$staridd);
			}
            fputcsv($file,$line);
        }
	...
	// ----------------------------------------------- 
    public function member_profilelist($id=null,$rowno=0){
		$data["member_list"] = "Management";
...
// Row per page 
   ...
   // All records count      
   $allcount = $this->Admin_model->getrecordCount();
    $adv = $this->input->post('adv_search');
    $advsearch=(isset($adv))?$adv:'';
   // Get  records      
   $users_record = $this->Admin_model->getData($rowno,$rowperpage,$advsearch);   
     //print_r($users_record);    
   // Pagination Configuration      
   $config['base_url'] = base_url().'adminmain/member_profilelist';      
  ...
  // ----------------------------------------------------------------------------- 
   ...
   // ------------------------------------------------
    ...
    public function userprofile_align()
    {
		...
		$proid=$this->input->post('palignment');
        $gender=$this->input->post('pgender');
		$data1 = array
		(
			'ProAlignment' => '9999'
		);
		$this->Admin_model->membesearch_index($data1,$proid,$gender);
    ...
    public function userimage_index()
    {
		...
		$proid=$this->input->post('pindexalignment');
		$gender=$this->input->post('pgender');
		$data1 = array
		(
			'IndexImage' => '8'
		);
		$this->Admin_model->membeprofile_index($data1,$proid,$gender);
		
		$data = array
		(
			'IndexImage' => $this->input->post('pindexalignment'),
		);
		$this->Admin_model->memberactive_ajax($data,$updateid);
		  redirect(base_url() . 'adminmain/member_profilelist/');
    }
  ...
  //----------------------------------------------
    ...
    public function review_submit($id=null){
		...
		$rating  = $this->input->post('rating');
			...
			$this->Admin_model->update_review($data,$updateid);
			if($updateid!=""){
			$config = array();
				$this->load->library('upload');
				$files   = $_FILES;
				...
				$_FILES['profile_image']['size']     = $files['profile_image']['size'][$i];
				$pre="REV";
				$path1 = $this->chsslibrary->Image_pref($pre);
				$path  = $path1 . "_" . $updateid;
				$file  = $files['profile_image']['name'][$i];
				...
				if (!$this->upload->do_upload('profile_image')) {
					...
					if ($file == "") {
						...
						$this->Admin_model->update_review($data1,$updateid);
						$directoy = $new_path . $newname;
						$this->chsslibrary->photoWatermark_WithResize($directoy, 294, 294);
					}
		...
		// ----------------------------------------------- 
    public function member_profiledata($id=null){
		$data["member_list"] = "Management";
		$userid = $this->session->userdata('adminlogged_in');
		$data["profile_details"]=$this->Admin_model->getprofiledetails();
		$data["Totalmember"]=$this->Admin_model->getprofiledetails();
    ...
    public function add_submit(){
       $data["member_list"] = "Management";
		...
		$today = $this->chsslibrary->call_date();
			...
			$updateid=$this->Admin_model->addreview($data);
			if($updateid!=""){
				$config = array();
				$this->load->library('upload');
				$files   = $_FILES;
	...
	{
	  if(empty($page)) { $pageNo =  1; } else { $pageNo = $page; }
	  
	  $this->load->library('pdf');
	  
	  $member = $this->Admin_model->getMembers($pageNo);
	  $html = '<html><style>table td, th {border: 1px solid #ddd;padding: 8px;} tr:nth-child(even){background-color: #f2f2f2;}table th {text-align: left;background-color: #04AA6D;color: white;}</style><body>';
	  $html .= '<h2>'.count($member).' - Memebers List</h2><table class="table" style="width: 100%; border: 1px solid #ccc;border-collapse: collapse; margin: 0px;"><tr><th width="10">S.No</th><th width="50">Code</th><th>Name</th> <th>Number</th> <th>Profile</th>';
	  foreach($member as $key => $mem)
	  {
	    $profile_image = $this->Admin_model->getprofileimageStatus_byid($mem->Id);
        if(count($profile_image)<=0 || $profile_image == "") { continue; }
        foreach($profile_image as $row) {
          $FilePath = $row->FilePath;
        }
	    $html .= '<tr><td>'.($key+1).'</td><td>'.$mem->MemberCode.'</td><td>'.$mem->Name.'</td><td>'.$mem->ContactNumber.'</td><td><img src="'.base_url('assets/profileimages/'.$FilePath).'" width="150"></td></tr>';
	  }
	  $html .= '</table></body></html>';
   	  $this->pdf->create($html,'Members-'.$pageNo);
