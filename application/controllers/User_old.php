...
class User extends CI_Controller
{
    ...
    public function register_step2()
    {
		...
		if($userid!=""){
			...
			$data = array(
				...
				'PermenantAddress' => $this->input->post('perm_address'),
			...
			$this->session->set_flashdata('message', 'Added successfully.');
			$userid=$this->chsslibrary->encoder($userid);
			$step=$this->chsslibrary->encoder("step3");
    ...
    public function register_step3()
    {
        ...
        $this->session->set_flashdata('message', 'Added successfully.');
		$userid=$this->chsslibrary->encoder($userid);
		$step=$this->chsslibrary->encoder("step4");
    ...
    public function register_step4()
    {
        ...
        $data = array(
			...
			'FatherStatus' => $father_alive,
			'FatherJob' =>$father_job,
			'MotherName' => $mother_name,
			'MotherStatus' => $mother_alive,
			'MotherJob' =>$mother_job,
			'OrderBirth' =>$this->input->post('orderof'),
...
<table style="table-layout:fixed;width:90%;width:600px;margin:0 auto;background:#fff;font-size:14px;border:2px solid #e8e8e8;text-align:left;table-layout:fixed;border-spacing:0">
			...
			<table style="table-layout:fixed;width:90%;width:600px;margin:0 auto;background:#fff;font-size:14px;border:2px solid #e8e8e8;text-align:left;table-layout:fixed;padding-bottom:67px;border-spacing:0;padding-left: 20px;padding-right: 20px;">
			<thead>
