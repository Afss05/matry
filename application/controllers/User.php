...
class User extends CI_Controller
{
    ...
    public function index(){
		...
		$data['userid']="";
		// $data['captchaImg'] = $this->renderCaptcha();
		$this->data = $data;	
	...
	// ------------------------------------------------------------------
    public function view_wedding($id=null){
		...
		$data['wedding_details']=$this->Wedding_model->getwed_byid($id);
		$data['profile_image']=$this->Wedding_model->getprofileimage_byid($id);
		...
		$this->template['topmenu'] = $this->load->view('home/top_menu', $this->data, true);
		$this->template['footer'] = $this->load->view('home/footer', $this->data, true);
		$this->load->view('admin/wedding/view', $this->template);
    }
    ...
    {
    	// $userCaptchaCode = $this->input->post('usercaptchaCode');
		// $sessCaptcha = $this->session->userdata('captchaCode');
		// if($userCaptchaCode != $sessCaptcha)
		// {
        ...
        //  'Birthtime' =>$this->input->post('FamilyType'),
...
//    }
    ...
    public function register_step2()
	{
		...
		if ($userid != "") {
			$disability = $this->input->post('disability') ?? "0";

			$doshamdetails = $this->input->post('doshamdetails');
			$doshamString = "";
			if (!empty($doshamdetails)) {
				foreach ($doshamdetails as $d) {
					$doshamString .= $d . ",";
				}
				$doshamString = rtrim($doshamString, ',');
			}
			$doshamdetails = $doshamString ?: "0";
			$doshamname = $this->input->post('doshamname') ?: "0";

			$father_name = $this->input->post('father_name') ?: "0";
			$mother_name = $this->input->post('mother_name') ?: "0";
			...
			if (isset($_FILES['profile_image']) && is_array($_FILES['profile_image']) && count($_FILES['profile_image']['name']) > 0) {
				...
				for ($i = 0; $i < $cpt; $i++) {
					...
					$path = $this->chsslibrary->profileImageName() . "_" . $userid;
					$file = $files['profile_image']['name'][$i];
					$ext = pathinfo($file, PATHINFO_EXTENSION);
					$newname = $ext ? $path . '.' . $ext : "";
			...
			if (isset($_FILES['horos_image']) && is_array($_FILES['horos_image']) && count($_FILES['horos_image']['name']) > 0) {
				$this->load->library('upload');
				$files = $_FILES;
				...
				for ($i = 0; $i < $cpt; $i++) {
					...
					$path = $this->chsslibrary->horscoImageName() . "_" . $userid;
    ...
    public function register_step5()
    {
		...
		if($userid!=""){
			...
			$Gender=$CastName=$Email=$mailName=$Name="";
	...
	<table style="table-layout:fixed;width:90%;width:750px;margin:0 auto;background:#fff;font-size:14px;border:2px solid #e8e8e8;text-align:left;table-layout:fixed;border-spacing:0">
			...
			$content = $this->chsslibrary->register_mail($email,$mailName);
  ...
  // -----------------------------------------------------------------------------    
	...
	$totalRecords=$this->Admin_model->review_total();
	...
	$data['review_details'] = $this->Admin_model->review_report($limit,$offset);
	...
	$this->template['topmenu'] = $this->load->view('home/top_menu', $this->data, true);
	$this->template['footer'] = $this->load->view('home/footer', $this->data, true);
    $this->load->view('home/review/review', $this->template);
	}
	...
	$this->template['topmenu'] = $this->load->view('home/top_menu', $this->data, true);
	$this->template['footer'] = $this->load->view('home/footer', $this->data, true);
    $this->load->view('home/review/review_add', $this->template);
	}
