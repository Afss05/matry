<?php
class Admin_model extends CI_Model
{
       
    public function __construct()
    {
		parent::__construct();	
		$this->load->library('Chsslibrary');
    }
   
   //duplicate record
//    DELETE FROM cm_citymaster WHERE Id NOT IN (SELECT * FROM (SELECT MIN(n.id) FROM cm_citymaster n GROUP BY n.CityName) x)
   
   public function verifyupd($userid,$status_id){
	   
	   $query=$this->db->query("update tr_member SET verified_status='$status_id' where Id='".$userid."'");
	   if($query){ return 1; }else{ return 0; }
   }
   
	public function plan_profileinactive(){
		
		$this->db->select('*');
		$this->db->from(TBL__PREFIX.'userpayment');
		$this->db->where('Status', '2');
		$query=$this->db->get();
		//echo $this->db->last_query();exit;
		$row = $query->result();
		
		foreach($row as $item){
			$id=$item->Id;
			$ActiveDates=$item->ActiveDates;
			$MemberId=$item->MemberId;
			$currentdate=$this->chsslibrary->call_date();
			if(strtotime($ActiveDates) < strtotime($currentdate)){
				
				
				$userid = $this->session->userdata('logged_in');
				$subscribedplan=$this->usersubscribedplan($userid);  

				$MProfileCounts=$countTototal="";
				$planid=$PaymentType=$MAmount=$MPaidedValidy="";
				if(count($subscribedplan)>0){
				foreach($subscribedplan as $plan){
				$planid=$plan->Id;
				$PaymentType=$plan->PaymentType;
				$MPaidedValidy=$plan->MPaidedValidy;
				$MProfileCounts=$plan->MProfileCounts;
				$MAmount=$plan->MAmount;

				}
				}
				
				$userdetails=$this->getprofiledetails_byid($userid);
				$phone=$email=$Name="";
				foreach($userdetails as $item){
				$Name=$item->Name;
				$email=$item->Email;
				$phone=$item->ContactNumber;
				}


				$paymentdetails=$this->getpayment_byid($PaymentType);
				$details="";
				foreach($paymentdetails as $pay){
				$statusId=$pay->Id;
				$details=$pay->PaymentType;
				}


				$countTototal=0;
				$yetviews=$this->getcurrent_paidviewcount($userid,$planid);  
				foreach($yetviews as $views){
				$countTototal=$views->totviewprofil;
				}

				$config = Array(
				'protocol' => 'smtp',
				'smtp_host' => 'smtp.admin@chennaicreativesolutions.com',
				'smtp_port' => 465,
				'smtp_user' => 'admin@chennaicreativesolutions.com', // change it to yours
				'smtp_pass' => 'ccs@9894323@', // change it to yours
				'mailtype' => 'html',
				'charset' => 'iso-8859-1',
				'wordwrap' => TRUE
				);
				$from_email = "admin@chennaicreativesolutions.com"; 

				$this->load->library('email'); 
				$this->email->from($from_email); 
				$this->email->to($email);
				$this->email->subject('Matrimony');  
				$content = $this->chsslibrary->user_plan_inactive($Name,$details,$MAmount,$MPaidedValidy,$MProfileCounts,$countTototal);
				//print_r($content);die;
				$this->email->message($content); 
				$this->email->send();
				
				
			$data = array
			(
			'Status' =>'3',
			);
			$this->db->set($data); 
			$this->db->where('Id', $id);
			$this->db->update(TBL__PREFIX.'userpayment',$data);
			
			$data1 = array(
			'Status' =>'0',
			);

			$this->update_profileviewcount($data1,$MemberId);
			
			}
		}
	} 
   
   
   
   
   
    //------------------------------------------------------------------------------        
    public function gettot_profiledetails($user,$password){  
        $this->db->select('*');
        $this->db->from(TBL__PREFIX.'adminlogin');
        $this->db->where('Username', $user);
        $this->db->where('Password', $password);
        $query = $this->db->get();
		// echo $this->db->last_query();exit;
        return $query->result();
    }
    //------------------------------------------------------------------------------        
    public function checkLogin($user,$password){  
        $this->db->select('*');
        $this->db->from(TBL__PREFIX.'adminlogin');
        $this->db->where('Username', $user);
        $this->db->where('Password', $password);
        $query = $this->db->get();
		// echo $this->db->last_query();exit;
        return $query->result();
    }
   
   //------------------------------------------------------------------------------        
    public function checkuserLogin($user,$password){  
        $this->db->select('*');
        $this->db->from(TBL__PREFIX.'member');
        $this->db->where("(Email = '$user' OR MemberCode = '$user')");
        $this->db->where('Password', $password);
        $query = $this->db->get();
		// echo $this->db->last_query();exit;
        return $query->result();
    }
  
    //------------------------------------------------------------------------------        
    public function checkOldPassword($old,$userid){
		$this->db->select('*');
        $this->db->from(TBL__PREFIX.'adminlogin');
        $this->db->where('Id', $userid);
		$this->db->where('Password', $old);
        $query = $this->db->get();
       // echo $this->db->last_query();exit;
        return $query->result();
    }
   
   //------------------------------------------------------------------------------
    public function updatepassword_byuser($data,$id){ 
        $this->db->set($data); //value that used to update column  
        $this->db->where('Id', $id); //which row want to upgrade  
        $this->db->update(TBL__PREFIX.'adminlogin',$data);
    }    
	
   
     //------------------------------------------------------------------------------    
    public function getMemberCount(){ 
        $this->db->select('MAX(Id) AS Totalcount');
        $this->db->from(TBL__PREFIX.'member');
        $sql=$this->db->get();
        return $sql->result();
    }
  //---------------------------------------------------   
    public function getMember_Count(){ 
        $this->db->select('MAX(Id) AS Totalcount');
        $this->db->from(TBL__PREFIX.'member');
		$query=$this->db->get();
		$row = $query->result();
		return $row;
    }
	  
	//--------------------------------------------------
	public function getprofiledetails(){ 
		$this->db->select('*');
		$this->db->from(TBL__PREFIX.'member');
		$this->db->order_by("Id", "desc");
		// $this->db->limit(30, 0); 
		$query=$this->db->get();
		//echo $this->db->last_query();exit;
		$row = $query->result();
		return $row;
	}


	// Fetch records 
 public function getData($rowno,$rowperpage,$advsearch) { 
 	//echo $advsearch;
 	//echo "r---".$rowno."t---".$rowperpage;
 	//echo TBL__PREFIX.'member';
 	//echo "r---".$rowno."t---".$rowperpage;
   /*$this->db->select('*'); 
   $this->db->from(TBL__PREFIX.'member');        
   $this->db->limit($rowperpage, $rowno);  
      if(isset($advsearch)){
   	$rr=trim($advsearch);
//$this->db->like('MemberCode',"%$rr%");

   }  
   $query = $this->db->get(); */  
$cond='';
   if(isset($advsearch)){
   	$rr=trim($advsearch);
$cond=" WHERE MemberCode like '%$rr%' or ContactNumber like '%$rr%'";

   } 
   if(empty($rowno)){
		$rowno=0;
   }      
  $sql = "select * from tr_member $cond ORDER BY Id DESC  LIMIT $rowno , $rowperpage";
$query = $this->db->query($sql);
   return $query->result(); 
 }

 // Select total records    
 public function getrecordCount() {
   $this->db->select('count(*) as allcount');      
   $this->db->from(TBL__PREFIX.'member');  
   $this->db->limit(1, 0);    
   $query = $this->db->get();      
   $result = $query->result_array();            
   return $result[0]['allcount'];    
 }



  public function getallmemberrecords() {
  /* $this->db->select('*');      
   $this->db->from(TBL__PREFIX.'member');     
   $query = $this->db->get();
   $this->db->limit(10, 0);     */   
    // $sql = "select m.Id,m.Name, m.ProfileFor, m.Email, m.MemberCode, m.Gender, m.Birthtime, m.Birthplace, m.DOB, m.Age, m.MotherTongue, c.CasteName, m.Gothram, m.Height, m.Qualification,m.Occupation, m.MonthlyIncome, m.ContactNumber, m.AlternativeNumber, m.PermenantAddress, m.StateId, m.CityId, m.PresentAddress, m.FatherName, m.FatherJob, m.MotherName, m.MotherJob, m.AboutMe, m.AddedDate,m.Star,m.Rasi,m.SubCaste,m.HDoshamDetails,m.MaritalStatus from tr_member m LEFT JOIN cm_caste c ON m.CastName=c.Id GROUP by m.Id"; 
    $sql = "select m.Id,m.Name, m.ProfileFor, m.Email, m.MemberCode, m.Gender, m.DOB, m.Age, m.MotherTongue, c.CasteName, m.Gothram, m.Height, m.Qualification,m.Occupation, m.MonthlyIncome, m.ContactNumber, m.AlternativeNumber, m.PermenantAddress, m.StateId, m.CityId, m.PresentAddress, m.FatherName, m.MotherName, m.AddedDate,m.Star,m.Rasi,m.SubCaste,m.HDoshamDetails,m.MaritalStatus from tr_member m LEFT JOIN cm_caste c ON m.CastName=c.Id GROUP by m.Id"; 
	$query = $this->db->query($sql);
   $result = $query->result_array();    
   foreach ($result as $key => $value) {
   	# code...
   	$memid=$value['Id'];
   	 $sql1 = "SELECT Id FROM `tr_userpayment` where MemberId='$memid' ORDER BY `Id` DESC LIMIT 1"; 
	$query1 = $this->db->query($sql1);
   $result1 = $query1->result_array();

   if(count($result1) > 0){
   	$paymentid=$result1[0]['Id'];
   	$sql2 = "SELECT count(Id) as totcount FROM `tr_profileviewcount` where PaIdedId='$paymentid' ORDER BY `Id` DESC LIMIT 1"; 
	$query1 = $this->db->query($sql2);
   $result2 = $query1->result_array();
   	$totcount=$result2[0]['totcount'];
   }else{
   	$totcount='';
   }
   //print_r($result1);
   	$result[$key]['paymenttype']=$totcount;
   	//echo $result['paymenttype'];
   //	echo print_r($value);
   }


   return $result;    
 }


		public function getprofiledetails1(){ 
		$this->db->select('*');
		$this->db->from(TBL__PREFIX.'member');
		$this->db->order_by("Id", "desc");
		$this->db->limit(30, 0); 
		$query=$this->db->get();
		//echo $this->db->last_query();exit;
		$row = $query->result();
		return $row;
	}
	
	
	//-----------------------related profiles---------------------
	
	public function related_profiles($ugender,$ucaste){
		if ($ugender=='M') {
			$applygender='F';
		} else {
			$applygender='M';
		}
		if (isset($ucaste)) {
			$order="ORDER BY FIELD(CastName,'$ucaste') DESC";
		}else{
			$order="ORDER BY Id DESC";
		}
		$sql = "select Id,MemberCode  from " . TBL__PREFIX . "member where Gender='".$applygender."' $order limit 0,4";
		//echo $sql;die;
		$query = $this->db->query($sql);
		$toaddimg=$query->result_array();
		foreach ($toaddimg as $key => $rr) {
			$id=$rr['Id'];
			$this->db->select('*');
			$this->db->from(TBL__PREFIX.'userprofile_photo');
			$this->db->where('MemberId', $id);
			$this->db->order_by("Id", "DESC");
			$this->db->limit(1,0);
			$query1=$this->db->get();
			//echo $this->db->last_query();exit;
			$row = $query1->result_array();	
			//print_r($row[0]);
			
			if (count($row)>0) {
				$toaddimg[$key]['single_img']=$row[0]['FilePath'];
			} else {
				$toaddimg[$key]['single_img']='';
			}
			//echo $row[0]['FilePath'];
			//$toaddimg[$key]['single_img']=$row[0]['FilePath'];
		}
		
		//print_r($toaddimg);
		return $toaddimg;	
		
	}

    //------------------------------------------------------------------------------
    public function getprofileimageStatus_byid($id){ 
        $this->db->select('*');
        $this->db->from(TBL__PREFIX.'userprofile_photo');
        $this->db->where('MemberId', $id);
		$this->db->where('Status','1');
		$this->db->limit('1');
        $query=$this->db->get();
        //echo $this->db->last_query();exit;
        $row = $query->result();
		return $row;
    }
   
   
   	//------------------------------------------------------------------------------
    public function deleteProfile($id){ 
      
		$this->db->from(TBL__PREFIX.'userprofile_photo');
		$this->db->where('MemberId', $id);
		$query=$this->db->get();
	
		$row = $query->result();
		
		
		//print_r($row); die;
		foreach($row as $item){
			$FilePath=$item->FilePath;
			$delete_path='assets/profileimages/'.$FilePath;
			if (file_exists('assets/profileimages/' . $FilePath)) {
				chmod($delete_path,0755);
				unlink("$delete_path");
				$this->db->where('MemberId', $id);
				   
				$this->db->delete(TBL__PREFIX.'userprofile_photo'); 
				  // echo $this->db->last_query();exit;
			}
			
		}
		$id=$this->db->where('Id', $id);
        $this->db->delete(TBL__PREFIX.'member'); 
    }    
	
   
   	// ------------------------------------------------------------------------------
    
    public function memberactive_ajax($data,$id)
    {
		$this->db->set($data); //value that used to update column  
        $this->db->where('Id', $id); //which row want to upgrade  
        $this->db->update(TBL__PREFIX.'member',$data);
		//echo $this->db->last_query();exit;
    }
   //------------------------------------------------------------------------------
	public function getpaymentdetails(){ 
		$this->db->select('*');
		$this->db->from(TBL__PREFIX2.'payment_master');
		$query=$this->db->get();
		//echo $this->db->last_query();exit;
		$row = $query->result();
		return $row;
	}
	
	  	// ------------------------------------------------------------------------------
    
    public function paymentactive_ajax($data,$id)
    {
		$this->db->set($data); //value that used to update column  
        $this->db->where('Id', $id); //which row want to upgrade  
        $this->db->update(TBL__PREFIX2.'payment_master',$data);
		//echo $this->db->last_query();exit;
    }
	
	//------------------------------------------------------------------------------    
    public function addpayment($data){  
		$this->db->insert(TBL__PREFIX2.'payment_master',$data); 
		//  echo $this->db->last_query();exit;
		return $this->db->insert_id();
    }
	
	
		//------------------------------------------------------------------------------        
    public function getpayment_byid($id){

        $this->db->select('*');
        $this->db->from(TBL__PREFIX2.'payment_master');
		$this->db->where('Id', $id);
        $query = $this->db->get();
      //echo $this->db->last_query();exit;
        return $query->result();
    }
	//------------------------------------------------------------------------------    
    public function update_paymentbyid($data,$id){  
        $this->db->set($data); 
        $this->db->where('Id', $id); 
        $this->db->update(TBL__PREFIX2.'payment_master',$data);
    }
	
	
	//------------------------------------------------------------------------------
	public function getreligondetails(){ 
		$this->db->select('*');
		$this->db->from(TBL__PREFIX2.'religion');
		$query=$this->db->get();
		//echo $this->db->last_query();exit;
		$row = $query->result();
		return $row;
	}
	
	
	 //------------------------------------------------------------------------------        
    public function checkemail_Byuser($email){

        $this->db->select('*');
        $this->db->from(TBL__PREFIX.'member');
        $this->db->where('Email', $email);
        $query = $this->db->get();
       //echo $this->db->last_query();exit;
        return $query->result();
    }
	
	//-----------------------------------------------------------       
    public function checkphone_Byuser($email){

        $this->db->select('*');
        $this->db->from(TBL__PREFIX.'member');
        $this->db->where('ContactNumber', $email);
        $query = $this->db->get();
       // echo $this->db->last_query();exit;
        return $query->result();
    }
	
	//-----------------------------------------------------------       
    public function checkphoneByuser($email,$uid){

        $this->db->select('*');
        $this->db->from(TBL__PREFIX.'member');
		$this->db->where_not_in('Id', $uid);
        $this->db->where('ContactNumber', $email);
        $query = $this->db->get();
        //echo $this->db->last_query();exit;
        return $query->result();
    }
	
	
	
	//-----------------------------------------------       
    public function getcasteby_religionajax($id){

        $this->db->select('*');
        $this->db->from(TBL__PREFIX2.'caste');
        $this->db->where('ReligionId', $id);
        $query = $this->db->get();
       //echo $this->db->last_query();exit;
        return $query->result();
    }
	
	//------------------------------------------------------------------------------
	public function getstatedetails(){ 
		$this->db->select('*');
		$this->db->from(TBL__PREFIX2.'statemaster');
		$query=$this->db->get();
		//echo $this->db->last_query();exit;
		$row = $query->result();
		return $row;
	}
	
	//------------------------------------------------------------------------------
	public function getcastedetails(){ 
		$this->db->select('*');
		$this->db->from(TBL__PREFIX2.'caste');
		$query=$this->db->get();
		//echo $this->db->last_query();exit;
		$row = $query->result();
		return $row;
	}
	
	//-------------------------------------------------
	public function getcitydetails(){ 
		$this->db->select('*');
		$this->db->from(TBL__PREFIX2.'citymaster');
		$this->db->order_by("CityName", "ASC");
		$query=$this->db->get();
		//echo $this->db->last_query();exit;
		$row = $query->result();
		return $row;
	}
	
	//------------------------------------------------------------------------------
	public function getstardetails(){ 
		$this->db->select('*'); 
		$this->db->from(TBL__PREFIX2.'star');
		$query=$this->db->get();
		//echo $this->db->last_query();exit;
		$row = $query->result();
		return $row;
	}
	//------------------------------------------------------------------------------
	public function getrasidetails(){ 
		$this->db->select('*');
		$this->db->from(TBL__PREFIX2.'rasi');
		$query=$this->db->get();
		//echo $this->db->last_query();exit;
		$row = $query->result();
		return $row;
	}
	
	//------------------------------------------------------------------------------        
    public function getstarby_rasiajax($id){

        $this->db->select('*');
        $this->db->from(TBL__PREFIX2.'starrasi');
        $this->db->where('RasiId', $id);
        $query = $this->db->get();
		//echo $this->db->last_query();exit;
        return $query->result();
    }
	
	
	
	//------------------------------------------------------------------------------    
    public function addmember($data){  
		$this->db->insert(TBL__PREFIX.'member',$data); 
		//echo $this->db->last_query();exit;
		return $this->db->insert_id();
    }
	
	
	//------------------------------------------------------------------------------    
    public function userprofile_add($data){  
	$this->db->insert(TBL__PREFIX.'userprofile_photo',$data); 
	//echo $this->db->last_query();exit;
	return $this->db->insert_id();
    }
	
	//------------------------------------------------------------------------------
    public function updateuserprofile($data,$id){ 
        $this->db->set($data); //value that used to update column  
        $this->db->where('Id', $id); //which row want to upgrade 
	
        $this->db->update(TBL__PREFIX.'member',$data);
			//echo $this->db->last_query();exit;	
    }    
	
	//------------------------------------------------------------------------------
	public function getprofiledetails_byid($id){ 
		$this->db->select('*');
		$this->db->from(TBL__PREFIX.'member');
		$this->db->where('Id', $id); 
		$query=$this->db->get();
		//echo $this->db->last_query();exit;
		$row = $query->result();
		return $row;
	}
	
	  //------------------------------------------------------------------------------
    public function getprofileimage_byid($id){ 
        $this->db->select('*');
        $this->db->from(TBL__PREFIX.'userprofile_photo');
        $this->db->where('MemberId', $id);
        $query=$this->db->get();
        //echo $this->db->last_query();exit;
        $row = $query->result();
		return $row;
    } 
	
	
	 //------------------------------------------------------------------------------
    public function deleteprofile_imagebyid($id){ 
	
		$this->db->select('*');
		$this->db->from(TBL__PREFIX.'userprofile_photo');
		$this->db->where('Id', $id);
		$query=$this->db->get();
		 //echo $this->db->last_query();exit;
		$row = $query->result();
		
		foreach($row as $item){
			$FilePath=$item->FilePath;
		}
		//echo $FilePath;die;
		if($FilePath!='')
		{
			$delete_path='assets/profileimages/'.$FilePath;
			chmod($delete_path,0755);
			unlink("$delete_path");
			$this->db->where('Id', $id);
			$this->db->delete(TBL__PREFIX.'userprofile_photo'); 
		}
 
    } 
	
	
	//------------------------------------------------------------------------------        
    public function getprofile_bygender($gender,$caste){

        $this->db->select('*');
        $this->db->from(TBL__PREFIX.'member');
        $this->db->where('Gender', $gender);
		$this->db->where('CastName', $caste);
		$this->db->limit('10');
        $query = $this->db->get();
		//echo $this->db->last_query();exit;
        return $query->result();
    }
	
	//------------------------------------------------------------------------------
	public function getpaymentdetailsbystatus(){ 
		$this->db->select('*');
		$this->db->from(TBL__PREFIX2.'payment_master');
		$this->db->where('Status','1');
		$query=$this->db->get();
		//echo $this->db->last_query();exit;
		$row = $query->result();
		return $row;
	}
	
	
	//------------------------------------------------------------------------------
	public function getpaymentdetailsbyamount_payid($payid,$am){ 
		$this->db->select('*');
		$this->db->from(TBL__PREFIX2.'payment_master');
		$this->db->where('Id',$payid);
		$this->db->where('Amount',$am);
		$query=$this->db->get();
		//echo $this->db->last_query();exit;
		$row = $query->result();
		return $row;
	}
	
	
	//------------------------------------------------------------------------------    
    public function adduser_payment($data){  
		$this->db->insert(TBL__PREFIX.'userpayment',$data); 
		//echo $this->db->last_query();exit;
		return $this->db->insert_id();
    }
	
	
	//------------------------------------------------------------------------------    
    public function getuser_paymentbystatus($id){  
		$this->db->select('*');
		$this->db->from(TBL__PREFIX.'userpayment');
		$this->db->where('MemberId',$id);
		$this->db->where('Status','2');
		$query=$this->db->get();
		//echo $this->db->last_query();exit;
		$row = $query->result();
		return $row;
    }
	//------------------------------------------------------------------------------
	public function getprofiledetails_byid_memcode($id,$memcode){ 
		$this->db->select('*');
		$this->db->from(TBL__PREFIX.'member');
		$this->db->where('Id', $id); 
		$this->db->where('MemberCode', $memcode); 
		$query=$this->db->get();
		//echo $this->db->last_query();exit;
		$row = $query->result();
		return $row;
	}
	 //------------------------------------------------------------------------------        
    public function checkUserOldPassword($old,$userid){
		$this->db->select('*');
        $this->db->from(TBL__PREFIX.'member');
        $this->db->where('Id', $userid);
		$this->db->where('Password', $old);
        $query = $this->db->get();
       // echo $this->db->last_query();exit;
        return $query->result();
    }
	
	//------------------------------------------------------------------------------        
    public function usersubscribedplan($id){
        $this->db->select('*');
        $this->db->from(TBL__PREFIX.'userpayment');
        $this->db->where('MemberId', $id);
		$this->db->where('Status','2');
        $query = $this->db->get();
      //echo $this->db->last_query()."<br>";
        return $query->result();
    }
	
	
	//------------------------------------------------------------------------------        
    public function checkinsert_memberid_viewid($id,$viewid){
     	$this->db->select('*'); 
        $this->db->from(TBL__PREFIX.'profileviewcount');
        $this->db->where('MemberId', $id);
		$this->db->where('ViewedId',$viewid);
        $query = $this->db->get();
		//echo $this->db->last_query();exit;
        return $query->result();
    }
	
	//------------------------------------------------------------------------------    
    public function setMemberProfileViewCount($data){  
		$this->db->insert(TBL__PREFIX.'profileviewcount',$data); 
		// echo $this->db->last_query();exit;
		return $this->db->insert_id();
    }
	
	//------------------------------------------------------------------------------        
    public function getprofile_paidviewcount($id){
     	$this->db->select('count(ViewCount) as totviewprofil'); 
        $this->db->from(TBL__PREFIX.'profileviewcount');
        $this->db->where('MemberId', $id);
		$this->db->where('Status','1');
        $query = $this->db->get();
       //echo $this->db->last_query();exit;
        return $query->result();
    }
	
	
	//------------------------------------------------ 
    public function getprevious_paidviewcount($id,$paidid){
     	$this->db->select('count(ViewCount) as totviewprofil'); 
        $this->db->from(TBL__PREFIX.'profileviewcount');
        $this->db->where('MemberId', $id);
		$this->db->where('PaIdedId', $paidid);
		$this->db->where('Status','0');
        $query = $this->db->get();
       //echo $this->db->last_query();exit;
        return $query->result();
	}
	
	
	
	  //------------------------------------------------------------------------------
    public function getprofile_byid($id){ 
        $this->db->select('*');
        $this->db->from(TBL__PREFIX.'member');
        $this->db->where('Id', $id);
        $query=$this->db->get();
        //echo $this->db->last_query();exit;
        $row = $query->result();
		return $row;
    } 
	
	 //--------------------------------------------------        
    public function getGroomlist(){  
	$sql="select * from ".TBL__PREFIX."member  where Gender='M'  ORDER BY `IndexImage` ASC LIMIT 0,4 ";
   // echo $sql; exit;
	$query = $this->db->query($sql);
	return $query->result(); 
    }
	
	//-------------------------------------------------        
    public function getBridelist(){  
		$sql="select * from ".TBL__PREFIX."member  where  Gender='F'  ORDER BY `IndexImage` ASC LIMIT 0,4 ";
		//echo $sql; exit;
		$query = $this->db->query($sql);
		return $query->result(); 	 
 
    }
	
	  //------------------------------------------------------------------------------
    public function update_planstatus($data,$id){ 
        $this->db->set($data); //value that used to update column  
        $this->db->where('MemberId', $id); //which row want to upgrade  
        $this->db->update(TBL__PREFIX.'userpayment',$data);
    }
	  //------------------------------------------------------------------------------
    public function update_profileviewcount($data,$id){ 
        $this->db->set($data); //value that used to update column  
        $this->db->where('MemberId', $id); //which row want to upgrade  
        $this->db->update(TBL__PREFIX.'profileviewcount',$data);
		//echo $this->db->last_query();exit;
    }
	
	//------------------------------------------------------------------------------
	public function getpaid_memberdetails(){ 
		$this->db->select('*');
		$this->db->from(TBL__PREFIX.'userpayment');
		$this->db->where('(Status="2" or  Status="3")');
		$query=$this->db->get();
		//echo $this->db->last_query();exit;
		
		return $query->result();
	}
	
	//------------------------------------------------       
    public function userwishlist_bymemid($id){
   
		$this->db->select('ViewedId')->from(TBL__PREFIX.'profileviewcount');
		$this->db->where('MemberId',$id); 
		$subQuery =  $this->db->get_compiled_select();
		// Main Query
		$this->db->select('*');
		$this->db->from(TBL__PREFIX.'member');
		$this->db->where("Id IN($subQuery)", NULL, FALSE);
		//$this->db->where('PStatus','1'); 
		//$this->db->where('PublishStatus','1'); 
		$this->db->order_by("Id", "desc");
		$query=$this->db->get();
		//echo $query = $this->db->last_query();  exit;
		return $query->result();
		
    }
	
	//-----------------------------------------------  
    public function userhorscop_profileadd($data){  
	$this->db->insert(TBL__PREFIX.'userhoroscope_photo',$data); 
	//echo $this->db->last_query();exit;
	return $this->db->insert_id();
    }
	
	//------------------------------------------------------------------------------
	public function checkuserhorscop_profile($id){ 
		$this->db->select('*');
		$this->db->from(TBL__PREFIX.'userhoroscope_photo');
		$this->db->where('MemberId',$id); 
		$query=$this->db->get();
		//echo $this->db->last_query();exit;
		return $query->result();
	}
	
	  //------------------------------------------------------------------------------
    public function updateuser_horoscop_bymemid($data,$id){ 
        $this->db->set($data); //value that used to update column  
        $this->db->where('MemberId', $id); //which row want to upgrade  
        $this->db->update(TBL__PREFIX.'userhoroscope_photo',$data);
    }  
	
	
	
	 //------------------------------------------------------------------------------
    public function deleteHoros_imagebyid($id){ 
	
		$this->db->select('*');
		$this->db->from(TBL__PREFIX.'userhoroscope_photo');
		$this->db->where('MemberId', $id);
		$query=$this->db->get();
		 //echo $this->db->last_query();exit;
		$row = $query->result();
		
		foreach($row as $item){
			$FilePath=$item->FilePath;
		}
		//echo $FilePath;die;
		if($FilePath!='')
		{
			$delete_path='assets/profileimages/'.$FilePath;
			chmod($delete_path,0755);
			unlink("$delete_path");
			$this->db->where('MemberId', $id);
			$this->db->delete(TBL__PREFIX.'userhoroscope_photo'); 
		}
 
    } 
	
	//------------------------------------------------------------------------------        
    public function getcurrent_paidviewcount($id,$paidid){
     	$this->db->select('count(ViewCount) as totviewprofil'); 
        $this->db->from(TBL__PREFIX.'profileviewcount');
        $this->db->where('MemberId', $id);
		$this->db->where('PaIdedId', $paidid);
		$this->db->where('Status','1');
        $query = $this->db->get();
      // echo $this->db->last_query();exit;
        return $query->result();
    }
	
	//------------------------------------------------ 
    public function userwishlist_bymemid_payid($id=null,$paid=null){
		$this->db->select('ViewedId')->from(TBL__PREFIX.'profileviewcount');
		$this->db->where('MemberId',$id); 
		$this->db->where('PaIdedId',$paid); 
		$subQuery =  $this->db->get_compiled_select();
		// Main Query
		$this->db->select('*');
		$this->db->from(TBL__PREFIX.'member');
		$this->db->where("Id IN($subQuery)", NULL, FALSE);
		//$this->db->where('PStatus','1'); 
		//$this->db->where('PublishStatus','1'); 
		$this->db->order_by("Id", "desc");
		$query=$this->db->get();
		//echo $query = $this->db->last_query();  exit;
		return $query->result();
    }
	//------------------------------------------------ 
    public function getprofil_viewcount_admin($id,$paidid){
     	$this->db->select('count(ViewCount) as totviewprofil'); 
        $this->db->from(TBL__PREFIX.'profileviewcount');
        $this->db->where('MemberId', $id);
		$this->db->where('PaIdedId', $paidid);
        $query = $this->db->get();
       //echo $this->db->last_query();exit;
        return $query->result();
	}
	// --------------------------
    
    public function memberpayment_ajax($data,$id)
    {
		$this->db->set($data); //value that used to update column  
        $this->db->where('Id', $id); //which row want to upgrade  
        $this->db->update(TBL__PREFIX.'userpayment',$data);
		//echo $this->db->last_query();exit;
    }
	
	
	//--------------------------------------
	public function getcomment_byid($id){ 
		$this->db->select('*');
		$this->db->from(TBL__PREFIX.'userpayment');
		$this->db->where('Id',$id); 
		$query=$this->db->get();
		//echo $this->db->last_query();exit;
		return $query->result();
	}
	
	
	//--------------------------------------------
	public function getreview(){ 
		$sql="SELECT * FROM ".TBL__PREFIX."user_review order by Id desc limit 4";
		//echo $sql;die;
		$query = $this->db->query($sql);
		return $query->result();
	}
	
	
		
	//----------------------------------------------   
    public function review_total()
    {
		$sql = "select *  from " . TBL__PREFIX . "user_review";
		//echo $sql;die;
		$query = $this->db->query($sql);
		return $query->num_rows();
    }
	
	
	//-----------------------------------------------
    // public function review_report($limit,$offset){ 
	
	// $tail = "";
	// $tail .= " limit $offset,$limit";

	// $sql="SELECT * FROM ".TBL__PREFIX."user_review  order by Id desc $tail ";
	
	// //echo $sql;die;
	// $query = $this->db->query($sql);
		
	// return $query->result();

    // }   
	public function review_report() {
    $sql = "SELECT * FROM ".TBL__PREFIX."user_review ORDER BY Id DESC";
    $query = $this->db->query($sql);
    return $query->result();
}

	
	//------------------------------------  
    public function addreview($data){  
		$this->db->insert(TBL__PREFIX.'user_review',$data); 
	// echo $this->db->last_query();exit;
		return $this->db->insert_id();
    }
	
	
	
	//-----------------------------------------------
    public function review_report_admin(){ 
	
	$sql="SELECT * FROM ".TBL__PREFIX."user_review order by Id desc ";
	//echo $sql;die;
	$query = $this->db->query($sql);
	return $query->result();
    }
	
	
	//----------------------------------------
    public function deleteReview($id){ 
		$id=$this->db->where('Id', $id);
        $this->db->delete(TBL__PREFIX.'user_review'); 
			//echo $this->db->last_query();exit;
    } 
	
	
	
	//--------------------------------------
	public function getreview_byid($id){ 
		$this->db->select('*');
		$this->db->from(TBL__PREFIX.'user_review');
		$this->db->where('Id',$id); 
		$query=$this->db->get();
		//echo $this->db->last_query();exit;
		return $query->result();
	}
	// --------------------------
    
    public function update_review($data,$id)
    {
		$this->db->set($data); //value that used to update column  
        $this->db->where('Id', $id); //which row want to upgrade  
        $this->db->update(TBL__PREFIX.'user_review',$data);
	//echo $this->db->last_query();exit;
    }
    
    	
	//------------------------------------------------------------------------------        
    public function getprofile_bygender_age($gender,$caste,$fage,$tage,$uage){

      	$tail='';
		
		if($gender!=""){
			$tail .= "  and ( Gender ='$gender' )";
		}
		if($caste!=""){
			$tail .= "  and  ( CastName ='$caste' ";
			
		}
		if($fage!="" && $fage!="0" && $tage!="" && $tage!="0" ){
			$tail .= "  or ( Age >= $fage and Age <= $tage)  )";
		}elseif($caste!=""){
			$tail .= "  ) ";
		}
	
		$sql="SELECT * FROM ".TBL__PREFIX."member 
		where PStatus='1' and PublishStatus='1' $tail limit 5";
	//echo $sql;die;
		$query = $this->db->query($sql);
		return $query->result();
    }
    
    
	// -------------------------------------------------------------------
    
    public function membeprofile_index($data,$id,$gender)
    {
		$this->db->set($data); //value that used to update column  
        $this->db->where('IndexImage', $id); //which row want to upgrade 
		$this->db->where('Gender', $gender); 		
        $this->db->update(TBL__PREFIX.'member',$data);
	//	echo $this->db->last_query();exit;
    }
    
    
    // ----------------------------------------------------------getprofileimageStatus_byid-------------
    
    public function membesearch_index($data,$id,$gender)
    {
		$this->db->set($data); //value that used to update column  
        $this->db->where('ProAlignment', $id); //which row want to upgrade  
    	$this->db->where('Gender', $gender); 	
        $this->db->update(TBL__PREFIX.'member',$data);
		//echo $this->db->last_query();exit;
    }

    function getMembers($page)
    {
      /*$this->db->select(TBL__PREFIX.'userprofile_photo.FilePath,'.TBL__PREFIX.'member.Name,'.TBL__PREFIX.'member.ContactNumber');
      $this->db->from(TBL__PREFIX.'member');
      $this->db->join(TBL__PREFIX.'userprofile_photo',TBL__PREFIX.'userprofile_photo.MemberId = '.TBL__PREFIX.'member.Id','INNER');
      $this->db->group_by(TBL__PREFIX.'member.Id');*/
      
      $page = ($page-1) * 10;
      $this->db->select('Id,Name,ContactNumber,MemberCode');
      $this->db->limit(10,$page);
      $this->db->order_by('Id','DESC');
      return $this->db->get(TBL__PREFIX.'member')->result();
    }

}

?>
