...
class Searchindex_model extends CI_Model
{
    ...
    public function getprofile_searchtotal($userReligionId,$usercaste,$MProfileCounts=0)
    {
		...
		$tail .= " and PStatus='1' and PublishStatus='1' ";
		...
		if($usercaste!=""){
			//$order ="ORDER BY CASE WHEN CastName='$usercaste' THEN '' END DESC";
			$order="ORDER BY FIELD(CastName,'$usercaste') DESC $tai11";
		}else{
			$order ="order by ProAlignment asc $tai11";
		}
	

    	$sql = "select *  from " . TBL__PREFIX . "member where 1=1 $tail $order";
		//echo $sql;die;
