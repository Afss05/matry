...
$query = $this->db->query($sql);
 ...
 public function getrecordCount() {
  ...
  /* $this->db->select('*');      
   ...
   $query = $this->db->get();
   $this->db->limit(10, 0);     */ 
    $sql = "select m.Id,m.Name, m.ProfileFor, m.Email, m.MemberCode, m.Gender, m.DOB, m.Age, m.MotherTongue, c.CasteName, m.Height, m.Qualification,m.Occupation, m.MonthlyIncome, m.ContactNumber, m.AlternativeNumber, m.PermenantAddress, m.StateId, m.CityId, m.PresentAddress, m.FatherName, m.MotherName, m.AddedDate,m.Star,m.Rasi from tr_member m LEFT JOIN cm_caste c ON m.CastName=c.Id GROUP by m.Id"; 
	$query = $this->db->query($sql);
   $result = $query->result_array();    
